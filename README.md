...
基于Node.js + Express的学术邮件发送系统，支持批量导师管理、网页内容抓取和自动邮件发送。
...
# 启动应用
pm2 start email_server.js --name "tutor-email-system"
...
```bash
# 构建和运行
docker build -t tutor-email-system .
docker run -d -p 3000:3000 --env-file .env tutor-email-system
```
...
```bash
# 实时查看日志
tail -f /var/log/tutor-email-system.log

# 日志轮转配置
# /etc/logrotate.d/tutor-email-system
/var/log/tutor-email-system.log {
    daily
    rotate 30
    compress
    delaycompress
